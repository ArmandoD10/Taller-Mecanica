...
switch ($action) {
    ...
    // 1. CABECERA (Actualizada para incluir la Sucursal)
    $sql_cabecera = "SELECT o.id_orden, DATE_FORMAT(o.fecha_creacion, '%d/%m/%Y %h:%i %p') as fecha_fmt,
                            ...
                            CONCAT('RD$ ', FORMAT(o.monto_total, 2)) as monto_total_fmt,
                            o.id_inspeccion,
                            s.nombre as sucursal_nombre -- <--- Agregamos el nombre de la sucursal
                     FROM orden o
                     JOIN inspeccion i ON o.id_inspeccion = i.id_inspeccion
                     JOIN vehiculo v ON i.id_vehiculo = v.sec_vehiculo
                     JOIN cliente c ON v.id_cliente = c.id_cliente
                     ...
                     JOIN orden_estado oe ON o.id_orden = oe.id_orden
                     JOIN estado e ON oe.id_estado = e.id_estado
                     JOIN sucursal s ON o.id_sucursal = s.id_sucursal -- <--- JOIN con la tabla sucursal
                     WHERE o.id_orden = ?
    ...
    // 2. TRABAJOS SOLICITADOS (Se mantiene igual)
    ...
    // 3. SERVICIOS Y TIEMPOS (Se mantiene igual)
    ...
    // 4. REPUESTOS UTILIZADOS (Se mantiene igual)
