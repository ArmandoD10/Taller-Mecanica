...
function listar_historial($conexion) {
    ...
    $sql = "SELECT 
                ...
                IF(p_cli.tipo_persona = 'Juridica', p_cli.nombre, CONCAT(p_cli.nombre, ' ', IFNULL(p_cli.apellido_p, ''))) AS cliente,
                CONCAT(m.nombre, ' ', mo.nombre, ' (', IFNULL(v.placa, v.vin_chasis), ')') AS vehiculo,
...
function obtener_detalle($conexion) {
    ...
    $sql = "SELECT 
                ...
                CONCAT(m.nombre, ' ', mo.nombre) AS vehiculo_desc,
